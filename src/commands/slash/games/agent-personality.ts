...
export default {
    data: new SlashCommandBuilder()
        .setName('agent-personality')
        .setDescription('Tes psikologi kocak: Agent Valorant apa yang paling cocok buat kamu?'),
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        const sendQuestion = async (qIndex: number, updateInteraction: any) => {
            if (qIndex >= questions.length) {
                ...
                // End of test - post publicly
                await updateInteraction.update({ content: 'Tes selesai! Mengirim hasil ke channel publik...', embeds: [], components: [] });
                await (interaction.channel as TextChannel).send({ content: `<@${interaction.user.id}> baru saja mengikuti tes kepribadian:`, embeds: [finalEmbed] });
                return;
            }
            ...
            if (updateInteraction === interaction) {
                await updateInteraction.editReply({ content: '', embeds: [embed], components: [row] });
            } else {
                await updateInteraction.update({ embeds: [embed], components: [row] });
            }
        };

        await sendQuestion(currentQuestion, interaction); // initiate first question

        const collector = interaction.channel?.createMessageComponentCollector({
            filter: i => i.user.id === interaction.user.id && i.customId.startsWith('pers_'),
            time: 60000 * 3, // 3 minutes full test timeout
        ...
        collector?.on('collect', async (i) => {
            ...
            await sendQuestion(currentQuestion, i);
        });
    },
};
