...
export default {
    name: 'guess-map',
    aliases: ['map', 'tebakmap'],
    description: 'Tebak nama Map dari clue clue yang diberikan',
    async execute(message: Message, args: string[]) {
        if (!message.channel) return;

        if (!setGameActive(message.channel.id, 'guess_map')) {
        ...
        collector.on('collect', async (m: Message) => {
            if (isAnswerCorrect(m.content, randomMap)) {
                ...
                try {
                    ...
                    await (message.channel as TextChannel).send(`Bener woy jawabannya **${randomMap}**! Tapi database poin lagi error 😭`);
                }
            }
        });
