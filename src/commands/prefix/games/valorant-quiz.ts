...
export default {
    name: 'valorant-quiz',
    aliases: ['quiz', 'kuis'],
    description: 'Kuis VALORANT pilihan ganda (Berhadiah Poin)',
    async execute(message: Message, args: string[]) {
        if (!message.channel) return;

        if (!setGameActive(message.channel.id, 'valorant_quiz')) {
        ...
        const descriptionLines = shuffledOptions.map((opt, i) => {
            ...
            const customId = `quiz_${message.id}_${i}_${isCorrect ? 'true' : 'false'}`;
            ...
            if (isCorrect) {
                ...
                try {
                    ...
                    row.components.forEach(c => {
                        const newBtn = ButtonBuilder.from(c as any).setDisabled(true);
                        if ((c as any).data.custom_id === correctCustomId) newBtn.setStyle(ButtonStyle.Success);
        ...
        collector.on('end', () => {
            ...
            if (!answered) {
                ...
                const disabledComponents = row.components.map(c => {
                    ...
                    if ((c.data as any).custom_id === correctCustomId) {
