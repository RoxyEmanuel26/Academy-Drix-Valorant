...
export default {
    name: 'agent',
    aliases: ['infoagent'],
    description: 'Tampilkan informasi lengkap mengenai agent Valorant (!agent <nama>)',

    async execute(message: Message, args: string[]) {
        const agentName = args.join(' ');
        if (!agentName) {
            return message.reply('❌ Harap berikan nama agent! Contoh: `!agent Jett`');
        }

        const waitMsg = await message.reply('🔄 Sedang mencari intel agent...');

        const catalog = getCatalog();
        if (!catalog.characters) {
            return waitMsg.edit('❌ Data catalog tidak tersedia saat ini.');
        }
        ...
        if (!agent) {
            return waitMsg.edit(`❌ Agent dengan nama **${agentName}** tidak ditemukan!`);
        }
        ...
        await waitMsg.edit({ content: null, embeds: [embed], files });
    }
};
